...
bt_save.addEventListener('click',(e) => {
    ...
    if (value == 1) {
        switch(count) {
            case 1:
                set(ref(db, 'Control_1/Device_2/TimeVal/'),{
        ...
        update(ref(db, 'Control_1/Device_2'), {
...
Mode_Ctrl.addEventListener('click', (e) => {
    ...
    update(ref(db, 'Control_1/Device_2'), { Mode_Ctrl: Mode_Ctrl});
});

// Realtime Mode Ctrl
const ref_modeCtrl = ref(db, 'Control_1/Device_2' + '/Mode_Ctrl');
let value;
...
bt_add.addEventListener('click',(e) => {
    ...
    update(ref(db, 'Control_1/Device_2'), { Counter: count});
});
...
// Realtime Counter 1
const ref_Counter1 = ref(db, 'Control_1/Device_2' + '/Counter');
...
SW_1.addEventListener('change',function(){
    if(this.checked) {
        update(ref(db, 'Control_1/Device_2/'), {TimeVal: 1 });
    } else {
        update(ref(db, 'Control_1/Device_2/'), {TimeVal: 0 });
    }
});

// Cập nhật trạng thái điều khiển thiết bị chế độ thủ công
const ref_sw1 = ref(db, 'Control_1/Device_2' + '/TimeVal');
